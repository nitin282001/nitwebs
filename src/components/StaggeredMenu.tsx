...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  position = 'right',
  colors = ['hsl(var(--primary-tint))', 'hsl(var(--primary))'],
  items = [],
  socialItems = [],
  displaySocials = true,
        ...
        <div className="sm-panel-inner">
          <ul className="sm-panel-list" role="list" data-numbering={displayItemNumbering || undefined}>
            {items && items.length ? (
              items.map((it, idx) => {
                if (it.isHeader) {
                  return (
                    <li className="sm-panel-itemWrap select-none border-b border-border/30 pb-1 mt-3 mb-1" key={it.label + idx}>
                      <span className="sm-panel-header-text select-none text-[11px] font-mono uppercase tracking-widest text-primary block">
                        {it.label}
                      </span>
                    </li>
                  );
                ...
                const itemClasses = `sm-panel-item ${it.isChild ? 'pl-4 text-[24px] font-medium tracking-normal text-secondary-text lowercase capitalize-first' : ''} ${it.isCta ? 'text-primary font-bold' : ''}`;
                ...
                return (
                  <li className="sm-panel-itemWrap" key={it.label + idx}>
                    <button
                      className={`${itemClasses} text-left w-full focus:outline-none`}
                      onClick={(e) => handleItemClick(it, e)}
                      aria-label={it.ariaLabel}
                      data-index={idx + 1}
                    >
          ...
          {displaySocials && socialItems && socialItems.length > 0 && (
            <div className="sm-socials" aria-label="Social links">
              <h3 className="sm-socials-title font-mono uppercase tracking-widest text-xs">Socials</h3>
