...
const resolveColorToRgbString = (colorStr: string): string => {
  // If it contains a CSS variable, resolve it
  ...
  // Handle space-separated HSL values (V4 syntax: hsl(0 0% 100%)) by replacing spaces with commas
  ...
  // Create a temporary canvas context to resolve using the browser engine
...
const SpecularButton: React.FC<SpecularButtonProps> = ({
  ...
  const resolvedLineColor = useMemo(() => {
    return resolveColorToRgbString(lineColor);
  }, [lineColor, themeTrigger]);

  const resolvedBaseColor = useMemo(() => {
    return resolveColorToRgbString(baseColor);
  }, [baseColor, themeTrigger]);
  ...
  useEffect(() => {
    ...
    // Only do work (layout reads, GPU draws) while the button is actually
    // on screen — with several of these mounted at once across a page,
    // an unconditional per-instance render/measure loop adds up fast.
    let isVisible = false;
    ...
    // Coalesce to at most one measurement pass per animation frame — native
    // pointermove can fire far faster than that, and each pass forces a
    // layout reflow via getBoundingClientRect.
